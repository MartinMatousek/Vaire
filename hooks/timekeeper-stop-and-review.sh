...
# timekeeper-session-end.sh (real session termination) and
# timekeeper-session-start.sh (when the user picks "Dokončit" on /clear,
...
on run argv
    ...
    try
        display dialog msg buttons {"OK", "Upravit"} default button "OK" with title "TimeKeeper — shrnutí"
