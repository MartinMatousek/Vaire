// Fills one timesheet "Log time" entry from a JSON argument and clicks Save —
...
// version created real duplicate entries in the user's live timesheet on
// repeated attempts, root cause never fully pinned down (suspected: a gap
// between the batch dedup check's snapshot and what the timesheet's UI
// actually reflects by the time each fill runs). Re-enabled
...
import { attachToTimesheetTab, runFillEntry } from './attach.mjs';
...
function parseEntryArg(argv) {
  ...
  if (!Number.isInteger(entry.minutes) || entry.minutes < 0 || entry.minutes > 45 || entry.minutes % 15 !== 0) {
    throw new Error(`minutes must be an integer 0, 15, 30, or 45 (the timesheet's own field limit), got ${JSON.stringify(entry.minutes)}`);
  }
  return entry;
}
